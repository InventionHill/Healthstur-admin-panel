...
import { LayoutDashboard, Users, Settings, LogOut, Menu, X, Activity, Dumbbell, Calendar, BarChart, CreditCard, Shield, ChevronLeft, ChevronRight, HelpCircle, Star } from 'lucide-react';
                ...
                <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto overflow-x-hidden">
                    {navItems.map((item) => (
                        <Link
                            ...
                            className={`group flex items-center py-2.5 rounded-lg text-white/70 hover:text-white hover:bg-white/5 transition-all duration-200 ${isDesktopCollapsed ? 'justify-center px-0' : 'px-3'}`}
                        >
                            <item.icon className={`flex-shrink-0 h-5 w-5 text-white/50 group-hover:text-white transition-colors ${isDesktopCollapsed ? '' : 'mr-4'}`} strokeWidth={1.5} />
                                ...
                                <nav className="px-2 space-y-1">
                                    {navItems.map((item) => (
                                        <Link
                                            ...
                                            href={item.href}
                                            onClick={() => setSidebarOpen(false)}
                                            className="group flex items-center px-2 py-2 text-base font-medium rounded-md text-gray-300 hover:bg-primary-hover hover:text-white transition-colors"
                                        >
                                            <item.icon className="mr-4 h-6 w-6 text-gray-400 group-hover:text-white transition-colors" />
                                            {item.name}
                                        </Link>
                                    ))}
                                </nav>
